...
class Preset:
    def calculate_scale(self, base_freq, mode="minor"):
        intervals = {
        ...
        steps = intervals.get(mode, intervals["minor"])
...
class Synth:
    ...
    def play_note(self, freq, duration=1.0):
        ...
        s2 = self.osc2.generate(freq*1.01, duration, 0.3)
        s3 = self.osc3.generate(freq*0.99, duration, 0.3)
        signal = s1 + s2 + s3
